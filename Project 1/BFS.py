...
class Board:
    ...
    def setBoard(self, grid, enemies, king, goals, rows, cols): # Sets the board with puzzle pieces
        # 1) Putting your king on the board
        king_y = king[0][1][0]
        king_x = king[0][1][1]
        self.board[king_y][king_x] = "Me"
        # 2) Putting obstacles on the board
        for y in range(rows):
            for x in range(cols):
                if grid[y][x] == -1:
                    self.board[y][x] = "Obstacle"
        # 3) Putting enemy pieces on the board + their threats
        for pair in enemies:
